...
from polytopax.algorithms.approximation import improved_approximate_convex_hull, compute_hull_quality_metrics
...
class TestImprovedAlgorithmBehavior:
    ...
    def test_approximation_quality_vs_exact_hull(self):
        ...
        a = 1.0
        h = a * jnp.sqrt(3) / 2
        triangle_points = jnp.array([
            [0.0, 0.0],      # A
            [a, 0.0],        # B
            [a/2, h]         # C
        ])
        exact_area = a * h / 2  # = a² * sqrt(3) / 4
