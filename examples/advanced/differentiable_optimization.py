...
def example_shape_optimization():
    ...
    def objective(points, target_area=1.0, target_centroid=jnp.array([0.5, 0.5])):
...
def example_multi_objective_optimization():
    ...
    for i, (w_area, w_comp) in enumerate(weight_combinations):
        ...
        objective = lambda p: multi_objective(p, w_area, w_comp)
...
def example_batch_optimization():
    ...
    print(f"\nFinal objectives:")
    for i, (initial, final) in enumerate(zip(initial_objectives, final_objectives)):
